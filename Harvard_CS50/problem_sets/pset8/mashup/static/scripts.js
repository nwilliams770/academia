...
$(document).ready(function() {
    ...
    // $('#map-canvas') returns a jQuery obj, get(0) gets the underlying DOM node
    // that jQuery is wrapping
    ...
    marker.addListener("click", function () {
        $.getJSON(Flask.url_for("articles"), { geo: place.postal_code }, function (data) {
            if (!$.isEmptyObject(data)) {
                const articlesList = "<ul>";
                data.forEach(function (row) {
                    articlesList += '<li><a href="' + row.link + '" target="_blank">' + row.title + '</a></li>';
                })
